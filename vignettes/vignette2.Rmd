...
We have seen that the genes differentially expressed aren't selected by their functionallity. However they could be more functionally related than the other genes. Are the differentially expressed genes more functionally similar than it would be expected ?
...
```
If we selected randomly the genes from our pool we would expect a score around `r scoreW` with a probability of `r p.val`. That means that the differentially expressed genes is highly different compared to the other genes if we use a significance threshold of 0.05 [^2].
...
```{r logfc1, fig.cap="Similarity of genes along abs(logFC). Assessing the similarity of genes according to their absolute log2 fold change.", fig.wide = TRUE}
...
```{r logfc2, fig.cap = "Functional similarity between the up-regulated and down-regulated genes."}
l <- sapply(s, function(x){
...
knitr::kable(cluster1, caption = "The similarity between miRNA")
```
