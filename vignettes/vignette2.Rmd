...
```{r newPathway2, fig.wide=TRUE, fig.cap="Comparison of functional similarities. In red the modified pathways.", warning=FALSE}
library("Hmisc")
genesReact2 <- genesReact
genesReact2[genes[deg]] <- sapply(genesReact[genes[deg]], function(x){c(x, "deg")})
...
gsGO <- GOSemSim::mgeneSim(rownames(gS), semData = BP, measure = "Resnik")
...
gsGOCC <- GOSemSim::mgeneSim(rownames(gS), semData = CC, measure = "Resnik")
...
gsGOMF <- GOSemSim::mgeneSim(rownames(gS), semData = MF, measure = "Resnik")
