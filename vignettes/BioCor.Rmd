...
As it development started aiming to improve clustering of genes by functionality in co-expression networks using `r BiocStyle::CRANpkg("WGCNA")` it also has some functions to combine similarities. 
...
## We remove genes which are not in list:
nan <- genes.id %in% names(genesReact)
react <- react[nan, nan]
...
clusters2 <- list(DNA = c("672", "675", "2073"),
                 RNA = c("16", "9128"),
                 acetil_metabolism = c("10", "15", "9", "644", "210", "1352"),
                 metabolism = c("18", "2893", "88"), 
                 unknown = c("2", "81327", "5142", "60"))
mycl <- cutree(hc, h = 0.8)
clusters <- split(names(mycl), as.factor(mycl))
names(clusters) <- c("acetil_metabolism", "RNA", "metabolism", "unknown", "DNA", "81")
