...
```{r simple_start, collapse = TRUE, eval = FALSE, cache = FALSE}
library("BioCor")
library("doParallel")
...
sim
```
Now in similarity we have a list of matrices of 20 rows and columns. 
...
This is covered by the GOSemSim package, you can use it to produce a similarity matrix (i.e. use 'r mgeneSim'). You can parallelize it with 'r foreach' package or BiocParallel if your list of genes is big. 
...
# To calculate the similarity we can us several functions

similarity <- addSimilarities(expr.adj, sim, w = c(0.8, 0.1, 0.1))
# Equivalent to the previous one
similarity <- Similarities(c(list(exp = expr.adj), sim), 
                           weighted, w = c(0.8, 0.1, 0.1)) 
# Multiply each similarity to keep the sign if we use TOMType = "signed""
similarity <- Similarities(c(list(exp = expr.adj), sim), prod) 
