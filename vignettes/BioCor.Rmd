...
It also provides some functions to combine several similarities and create a new similarity of several ones. As it development was started aiming to better group genes by functionallity in co-expression networks using [WGCNA](https://cran.r-project.org/package=WGCNA).
...
genes.react <- select(reactome.db, keys = entrezids, keytype = "ENTREZID", 
                      columns = "REACTOMEID")
corPathways(c("81", "18"), genes.kegg, "ENTREZID", "PATH")
corPathways(c("81", "18"), genes.react, "ENTREZID", "REACTOMEID")
```

If we want multiple pair-wise comparisons we can use bioCor, which accepts a parallel back-end:
```{r big, collapse=TRUE, eval=TRUE, cache=FALSE}
...
This is implemented in the `comparePathways` function, which results is similar to Jaccard index:

$J(x, w) = \frac{|x \cap w|}{|x \cup w|}$

The resulting values are $sim(x, w)  \in [0, 1]$. To calculate the similarity between two genes thus is:
$corGenes(i, j) = max(sim(x_i, w_j))$

Which is implemented in the `corGenes` function.
...
## Why don't you use other function to combine scores between two genes?
Although I considered to borrow the `combineScores` function from `GOSemSim` to combine similarities it was designed for similarity matrices not similarity values. And the above reasons made me use the max score.
