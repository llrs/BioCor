...
This method of comparing clusters requires two methods to combine values, the first one for combining pathways similarities and the second one to combine genes. If only one is provided it returns the matrix of similarities of the genes of each cluster:
```{r clusterGeneSim}
...
Note the differences between each method in the similarity values of the clusters except if we set `method = c("max", "max")`, then the similarity between the clustrs is the same as `clusterSim`.
...
Each method results in a different dendrogram as we can see  on Figure \@ref(fig:hclust3) compared to Figure \@ref(fig:hclust3b), but the second is more accurate to describe the functional relationships between the genes in the clusters.
...
[WGCNA](https://cran.r-project.org/package=WGCNA) uses the correlation of the expression data of several samples to cluster genes. Sometimes, from a biological point of view the interpretation of the resulting modules is difficult, even more when some groups of genes end up not having an enrichment in previously described functions. 
...
For these reasons it is better to use weights between `0.5` and `1` for expression if you use `weighted.sum`.
