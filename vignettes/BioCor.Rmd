...
As it development started aiming to improve clustering of genes by functionality in weighted gene co-expression networks using `r BiocStyle::CRANpkg("WGCNA")`. The package has some functions to combine similarities in order to integrate with `WGCNA`. 
...
The function `combineScoresPar` allows to use a parallel background (using `r Biocpkg("BiocParallel")`) to combine the scores. It is recommended to use a parallel background if you calculate more than 300 gene similarities. It also have an argument in case you want to calculate the similarity scores of severeal sets.
