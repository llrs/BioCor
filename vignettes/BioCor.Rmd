...
As it development started aiming to improve clustering of genes by functionality in weighted gene co-expression networks using `r BiocStyle::CRANpkg("WGCNA")` it also has some functions to combine similarities. 
...
genesReact <- sapply(genesReact, function(x){grep("R-HSA-", x, value = TRUE)}) # Only for human pathways
genesReact <- genesReact[lengths(genesReact) >= 1] # Remove genes which are not from human organism
```
...
We can set a weight to each similarity input with `weighted.sum`, multiply them also using a weight for each similarity (with `weighted.prod`), doing the mean or just adding them up. similarities allow us to apply a function to combine the matrices of a list.
```{r merging}
...
(clusters <- clusters[lengths(clusters) >= 2] )# Removing clusters of just one gene
...
plot(hclust(as.dist(1 - sim_clus1)), main = "Similarities between clusters by pathways")
```

```{r hclust3b, fig.cap="Clustering using clusterGeneSim", fig.wide = TRUE}
sim_clus2 <- mclusterGeneSim(clusters, genesReact)
plot(hclust(as.dist(1 - sim_clus2)), main ="Similarities between clusters by genes")
```
...
In this section I will compare the functional similarity of BioCor with the closely related package `r Biocpkg("GOSemSim")`. The genes and gene clusters used were extracted from GOSemSim's vignette.
...
```{r geneSimGOSemSim}
geneSim("241", "251", genesKegg, "BMA") - go2
geneSim("241", "251", genesReact, "BMA") - go2
BioCor::mgeneSim(c("835", "5261","241", "994"), genesKegg, "BMA", round = TRUE)
mgeneSim(c("835", "5261","241", "994"), genesReact, "BMA", round = TRUE) - go4
```
We can observe that for the first genes there isn't information on the pathways of those genes in the Kegg data set.
...
GOSemSim doesn't compare the clusters by comparing each gene but by comparing the gene ontologies. Thus we use the mclusterSim which is the equivalent.
...
There are several things to take into account when choosing a way to combine:
 - The size of the grey or 0 modules (those who don't show a specific pattern)
...
## I get an error! How do I solve its?
