...
# We can remove colums with too much NA
...
Now we add the two genes mores the BRCA1 and BRCA2 which have the entrez identification 672 and 675, respectively.
```{r hclust2} 
...
To choose how to select a combine method, we propose the following rules, build several networks with different combinations and pick the one which produces more groups or whose modules are more randomly related or where the 0 modules or the grey modules has the less number of genes. 
