...
And use the main function, `bioCor` which returns a list of matrices:
```{r small}
bioCor(c("81", "18"), react = TRUE, kegg = FALSE) # The default, only from Reactome
bioCor(c("81", "18"), react = FALSE, kegg = TRUE) # Only from KEGG
bioCor(c("81", "18"), all = TRUE) # Both
```

To speed calculation we can set up a parallel back-end:
...
This is similar to Jaccard index:
...
It keeps the one with the higher similarity. Although I might borrow the `combineScores` function from GOSemSim to combine similarities. 
